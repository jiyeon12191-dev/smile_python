...
info = { "name" : "이순신" , "age":20, "job":"군인","name":"김유신"} # 동일한 키를 사용할경우 덮어쓰기가 됨 
print( info["name"])
...
if "김유신" in info.values(): # 딕셔너리에서 value 가져오기 
    print("김유신있다")
...
# 5000원 이하는 저가형, 5000원 초과는 고가형 상품 이라고 저장 하시오 
# if~else 를 표현식에 쓰는경우
...
for i in range(len(snackName)):
    snNa[snackName[i]] = price[i]
for k,v in snNa.items():
    print(k,v)

# 문제2. 딜셔너리를 만들고 다음 조건으로 조회 하세요 
...
# 딕셔너리 키는 제품명 value는 브랜드와 금액 
# 삼성 브랜드의 제품명과 금액을 출력 하세요 
...
#     product[item[i]] = brand[i] , price[i]

for k,v in product.items():    
    if v["brand"] == "삼성":
        print(k,v["price"])

# set - 중복 허용하지 않고, 순서가 없는 자료형 
# 데이터 정리, 검색 
...
# c = {"바나나","참외","사과","귤","포도","딸기","토마토"} # 현규가 좋아 하는 과일 
...
# print( a ^ c ) # 대칭 차집합 (a에서c 공통된것을 뺀 나머지) ^(xor) = and or not 합친거 

# for fruit in c: 
#     print(fruit)

# '''
#     list - 다수의 데이터 저장용으로 많이 사용 , 중복허용, 추가 수정 삭제 가능 
#     tuple - 다수의 데이터 저장 기능, 중복허용, 추가수정 삭제 불가능 
...
while True:
    ...
    if menu == "1":
        ...
        found = False # 판매한 과일 이 과일 데이터에 있냐 없냐 
        for fruit in fruits:
            if fruit["name"] == name:
                found = True
                if fruit["stock"] >= count:
                  ...
                  # 판매 내역을 튜플로 생성  
                    ...
                    sales.append(sale) # 판매 기록용 sales에 저장  
                    print("판매 완료")
                    print("총 금액:", fruit["price"]* count,"원")
                else:
                    print("재고가 부족합니다")      
            
            if not found: # 과일데이터에 판매할 과일이 없는경우 - not연산자에 의해 거짓 <-> 참
                print("해당 과일이 없습니다")   
       ...
       # 오늘의 추천 과일 - 랜덤 
       # import random - randint(1,10): 1~10 사이정수 , randrange(1,10): 1~9 사이 정수
       # choice(리스트) : 리스트 내부의 값중 하나 선택 
       # choices(리스트, k=3):리스트 안에서 3개를 선택(중복허용)
       # sample(리스트,4) : 리스트 안에 값중 4개를 선택 (중복 없음)
       # shuffle(리스트) : 리스트안에 값의 순서를 랜덤하게 섞음 
       # choices( 리스트, 가중치 , k=5) : 리스트안에 값의 확률(가중치)을 적용하여 5개 선택 
       # i=[1,2,3,4,5]
       # w = [0.2,0.3,0.2,0.1,0.2]
       # choices( i , w, k=3 )

       
       # random.seed(10) # 처음 시작점 seed 
       # print( random.randint(1, 100 ) )  

        recommend = random.sample( fruits , 3 )
        # recommend.sort()
        # sorted 는 정렬된 새로운 리스트 반환한다 
